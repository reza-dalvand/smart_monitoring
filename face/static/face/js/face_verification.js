(function () {
    ...
    const FACE_STATE = {
        NO_FACE: 'NO_FACE', MULTIPLE_FACES: 'MULTIPLE_FACES', OUT_OF_FRAME: 'OUT_OF_FRAME',
        TOO_SMALL: 'TOO_SMALL', TOO_LARGE: 'TOO_LARGE', VALID: 'VALID', STABLE: 'STABLE',
    };

    const CHALLENGE_STATE = { IDLE: 'IDLE', ACTIVE: 'ACTIVE', PAUSED: 'PAUSED', COMPLETED: 'COMPLETED' };
    const VERIFICATION_STATE = {
        NOT_STARTED: 'NOT_STARTED', IN_PROGRESS: 'IN_PROGRESS', CAPTURING: 'CAPTURING',
        SUBMITTING: 'SUBMITTING', VERIFIED: 'VERIFIED', FAILED: 'FAILED', EXPIRED: 'EXPIRED', CAMERA_ERROR: 'CAMERA_ERROR',
    };
    ...
    function mapErrorMessage(code) { return ERROR_MESSAGES[code] || 'خطا در احراز هویت چهره.'; }
    ...
    class CameraController {
        constructor(videoEl) { this.videoEl = videoEl; this.stream = null; }
        async start() {
            try {
    ...
    class FaceLandmarkerController {
        constructor(config) { this.config = config; this.landmarker = null; this.ready = false; }
        async init() {
        ...
        detect(video, timestamp) {
            if (!this.ready || !this.landmarker || !video || video.readyState < 2) return { faceCount: 0, landmarks: [], blendshapes: [] };
            try {
                const result = this.landmarker.detectForVideo(video, timestamp);
                return { faceCount: (result.faceLandmarks || []).length, landmarks: result.faceLandmarks || [], blendshapes: result.faceBlendshapes || [] };
            } catch (e) { return { faceCount: 0, landmarks: [], blendshapes: [] }; }
        }
        close() { try { if (this.landmarker && this.landmarker.close) this.landmarker.close(); } catch (e) {} this.ready = false; }
    }

    class FacePositionValidator {
        constructor(guide) { this.guide = guide; }
        computeBBox(landmarks) {
            if (!landmarks || !landmarks.length) return null;
            let minX = 1, minY = 1, maxX = 0, maxY = 0;
            for (const lm of landmarks) { if (lm.x < minX) minX = lm.x; if (lm.y < minY) minY = lm.y; if (lm.x > maxX) maxX = lm.x; if (lm.y > maxY) maxY = lm.y; }
            return { x: minX, y: minY, width: maxX - minX, height: maxY - minY };
        }
        validate(faceResult) {
            if (!faceResult || faceResult.faceCount === 0) return { state: FACE_STATE.NO_FACE, message: 'صورت خود را مقابل دوربین قرار دهید' };
            if (faceResult.faceCount > 1) return { state: FACE_STATE.MULTIPLE_FACES, message: 'فقط یک نفر باید مقابل دوربین باشد' };
            const bbox = this.computeBBox(faceResult.landmarks[0]);
            if (!bbox) return { state: FACE_STATE.NO_FACE, message: 'صورت خود را مقابل دوربین قرار دهید' };
            const g = this.guide;
            if (bbox.width < g.minWidth || bbox.height < g.minHeight) return { state: FACE_STATE.TOO_SMALL, message: 'کمی به دوربین نزدیک‌تر شوید' };
            if (bbox.width > g.maxWidth || bbox.height > g.maxHeight) return { state: FACE_STATE.TOO_LARGE, message: 'کمی از دوربین فاصله بگیرید' };
            const insideX = bbox.x >= g.minX && (bbox.x + bbox.width) <= g.maxX;
            const insideY = bbox.y >= g.minY && (bbox.y + bbox.height) <= g.maxY;
            if (!insideX || !insideY) return { state: FACE_STATE.OUT_OF_FRAME, message: 'صورت خود را در کادر قرار دهید' };
    ...
    class HeadPoseDetector {
        avgLandmarks(landmarks, indices) {
            let x = 0, y = 0, count = 0;
            for (const idx of indices) { const lm = landmarks[idx]; if (!lm) continue; x += lm.x; y += lm.y; count += 1; }
            if (!count) return null; return { x: x / count, y: y / count };
        }
        estimateHeadPose(landmarks) {
            ...
            const nose = landmarks[1]; const forehead = landmarks[10]; const chin = landmarks[152];
            if (!rightEye || !leftEye || !nose || !forehead || !chin) return null;
            const eyeVecX = leftEye.x - rightEye.x; const eyeVecY = leftEye.y - rightEye.y;
            const eyeDist = Math.hypot(eyeVecX, eyeVecY);
            if (eyeDist < 0.01) return null;
            const unitLeftX = eyeVecX / eyeDist; const unitLeftY = eyeVecY / eyeDist;
            const centerX = (leftEye.x + rightEye.x) / 2; const centerY = (leftEye.y + rightEye.y) / 2;
            const noseVecX = nose.x - centerX; const noseVecY = nose.y - centerY;
            ...
            const faceHeight = Math.abs(chin.y - forehead.y) || eyeDist * 2.2;
            const pitch = ((nose.y - centerY) / faceHeight) * 90;
    ...
    class BlinkDetector {
        constructor(config) { this.config = config; this.state = 'open'; this.closedAt = null; }
        reset() { this.state = 'open'; this.closedAt = null; }
        getScore(categories) {
        ...
        update(score, timestamp) {
            ...
            if (this.state === 'open' && score >= this.config.closureThreshold) { this.state = 'closed'; this.closedAt = timestamp; return false; }
    ...
    class EvidenceCollector {
        ...
        maybeCapture(timestamp, stage) {
            ...
            const maxWidth = 640; const scale = Math.min(1, maxWidth / this.videoEl.videoWidth);
    ...
    class VerificationUI {
        ...
        setInstruction(message) {
            ...
            this.instruction.classList.remove('fv-hidden'); this.instruction.textContent = message;
        }
        setTimer(ms) {
        ...
        showResult(type, title, message, canRetry = false) {
            ...
            this.resultIcon.innerHTML = type === 'success' ? '<i class="bi bi-check-circle-fill"></i>' : '<i class="bi bi-x-octagon-fill"></i>';
            this.resultTitle.textContent = title; 
            this.resultMessage.textContent = message;
            this.retryBtn.classList.toggle('fv-hidden', !canRetry);
        }
        hideCameraControls() { this.setInstruction(null); this.showOverlay(false); }
    }

    class VerificationController {
        constructor(config) {
            this.config = config; this.ui = new VerificationUI();
            this.camera = new CameraController(this.ui.video); this.landmarker = new FaceLandmarkerController(config);
            this.api = new VerificationApi(config.csrfToken, config.startUrl, config.completeUrlBase);
            this.session = null; this.dynamicConfig = null; this.validator = null;
            this.headPose = new HeadPoseDetector(); this.blinkDetector = null; this.evidence = null;
            this.headPoseConfig = {};
            this.faceState = FACE_STATE.NO_FACE; this.challengeState = CHALLENGE_STATE.IDLE;
            this.verificationState = VERIFICATION_STATE.NOT_STARTED;
            this.stableSince = null; this.currentStepIndex = 0;
            this.stepConditionSince = null; this.challengeSequence = [];
            this.clockOffset = 0; this.loopTimer = null;
            this.timerInterval = null; this.isBusy = false; this.isTerminated = false;
        ...
        async init() {
            try {
                ...
                this.startTimer(); this.startLoop();
            } catch (error) {
        ...
        async startSession() {
            ...
            // مقادیر پیش‌فرض ایمن در صورت عدم ارسال از سمت سرور
        ...
        startLoop() {
            ...
            this.loopTimer = setInterval(() => {
                ...
                } catch (e) {} finally { this.isBusy = false; }
            }, interval);
        }
        ...
        processFrame(result, timestamp) {
            ...
            if (position.state !== FACE_STATE.VALID) {
                this.faceState = position.state; this.stableSince = null; this.stepConditionSince = null;
                if (this.blinkDetector) this.blinkDetector.reset();
                this.ui.setGuide('red'); this.ui.setStatus(position.message, 'danger');
                if (this.challengeState === CHALLENGE_STATE.ACTIVE) {
                    this.challengeState = CHALLENGE_STATE.PAUSED; this.ui.setInstruction('صورت خود را دوباره در کادر قرار دهید');
                }
                return;
            }
            this.faceState = FACE_STATE.VALID; this.ui.setGuide('green');
            if (!this.stableSince) { this.stableSince = timestamp; this.ui.setStatus('چهره در موقعیت مناسب است', 'success'); return; }
            const stableMs = this.dynamicConfig.stableDurationMs || 700;
            if (timestamp - this.stableSince < stableMs) { this.ui.setStatus('چهره در موقعیت مناسب است', 'success'); return; }
            this.faceState = FACE_STATE.STABLE; this.ui.setStatus('چهره آماده است', 'success');
            if (this.challengeState === CHALLENGE_STATE.IDLE) this.startChallenge();
            if (this.challengeState === CHALLENGE_STATE.PAUSED) {
                this.challengeState = CHALLENGE_STATE.ACTIVE; this.stepConditionSince = null;
        ...
        startChallenge() {
            this.challengeState = CHALLENGE_STATE.ACTIVE; this.currentStepIndex = 0;
            this.stepConditionSince = null; this.ui.setProgress(0, 0);
            this.ui.setInstruction(this.getCurrentInstruction());
        }
        ...
        processChallenge(result, timestamp) {
            ...
            if (step === 'TURN_LEFT' || step === 'TURN_RIGHT') {
                ...
                const correctCondition = step === 'TURN_LEFT' ? pose.yaw <= -minYaw : pose.yaw >= minYaw;
                const wrongCondition = step === 'TURN_LEFT' ? pose.yaw >= minYaw : pose.yaw <= -minYaw;
                if (wrongCondition) {
                    this.stepConditionSince = null;
                    this.ui.setInstruction(step === 'TURN_LEFT' ? '⚠️ جهت اشتباه است! لطفاً به سمت چپِ خودتان بچرخانید' : '⚠️ جهت اشتباه است! لطفاً به سمت راستِ خودتان بچرخانید');
                    return;
                }
                if (correctCondition) {
                    ...
                    else this.ui.setInstruction('✅ عالی است! حالت را نگه دهید...');
                } else {
                    this.stepConditionSince = null; this.ui.setInstruction(this.getCurrentInstruction());
                }
                return;
            }
            if (step === 'BLINK') {
                const blendCategories = result.blendshapes && result.blendshapes[0] ? result.blendshapes[0].categories : null;
        ...
        completeStep() {
            this.currentStepIndex += 1; this.stepConditionSince = null;
            if (this.blinkDetector) this.blinkDetector.reset();
            if (this.currentStepIndex >= this.challengeSequence.length) this.completeChallenge();
            else { this.ui.setProgress(this.currentStepIndex, this.currentStepIndex); this.ui.setInstruction(this.getCurrentInstruction()); }
        }

        completeChallenge() {
            this.challengeState = CHALLENGE_STATE.COMPLETED; this.verificationState = VERIFICATION_STATE.CAPTURING;
            this.ui.setProgress(this.challengeSequence.length, this.challengeSequence.length);
            this.ui.setInstruction('در حال جمع‌آوری شواهد...'); this.ui.setStatus('چالش‌ها کامل شد. لطفاً چند لحظه صبر کنید.', 'info');
        }
        ...
        buildChallengeResult() {
            const result = {}; for (const step of this.challengeSequence) result[step] = true; return result;
        }
        ...
        async submitEvidence() {
            ...
            this.ui.setInstruction(null); this.ui.setStatus('در حال ارسال شواهد به سرور...', 'info');
            this.ui.showOverlay(true, 'در حال پردازش نهایی...');
            const frames = this.evidence.getFrames(); const challengeResult = this.buildChallengeResult();
            try {
