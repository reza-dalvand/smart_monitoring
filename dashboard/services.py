...
def generate_questions_for_job(job: AIGenerationJob, count: Optional[int] = None):
    ...
    try:
        ...
        educational_context = (
            job.prompt if job.prompt and job.prompt.strip()
            else f"سرفصل رسمی درس {course} پایه {grade} رشته {field}"
        )

        # ✅ فقط یک بار سرویس صدا زده شود
        ...
        job.status = 'failed'
        job.error_message = f"موضوع نامعتبر: {_get_error_message(e)}"
        logger.error(f"Topic validation failed: {e}")

    except InsufficientContentError as e:
        job.status = 'completed'
        job.error_message = _get_error_message(e)
